...
import Jolokia, { JolokiaErrorResponse, JolokiaFetchErrorResponse, JolokiaRequest, JolokiaResponse, JolokiaSuccessResponse } from 'jolokia.js'
...
export const AiView: React.FunctionComponent = () => {
  ...
  if (!loaded) {
    return (
      <PageSection hasBodyWrapper={false}>
        <Spinner aria-label='Loading AI tree' />
      </PageSection>
    )
  }
  ...
  return (
    <AIViewContext.Provider value={{ tree, selectedNode, setSelectedNode }}>
...
const AITreeView: React.FunctionComponent = () => {
  const { tree, selectedNode, setSelectedNode } = useContext(AIViewContext)
...
const AIContentView: React.FunctionComponent = () => {
  const { selectedNode } = useContext(AIViewContext)
...
const MemoryView: React.FunctionComponent = () => {
  ...
  useEffect(() => {
    ...
    const register = async (request: JolokiaRequest, callback: (response: JolokiaResponse | JolokiaErrorResponse | JolokiaFetchErrorResponse) => void) => {
      handle = await jolokiaService.register(request, callback)
      log.debug(selectedNode.name, '- Register request: handle =', handle)
    }
    register({ type: 'read', mbean, attribute: ['HeapMemoryUsage', 'NonHeapMemoryUsage'] }, (response: JolokiaResponse | JolokiaErrorResponse | JolokiaFetchErrorResponse) => {
      ...
      const successResponse = response as JolokiaSuccessResponse
      log.debug(selectedNode.name, '- Scheduler - Attributes:', successResponse.value)
      const attrs = successResponse.value as AttributeValues
      setAttributes(attrs)
    })
...
const OSView: React.FunctionComponent = () => {
  ...
  useEffect(() => {
    ...
    register(
      ...
      (response: JolokiaResponse | JolokiaErrorResponse | JolokiaFetchErrorResponse) => {
        if (Jolokia.isResponseError(response) || Jolokia.isResponseFetchError(response)) {
          log.error(selectedNode.name, '- Scheduler - Error:', response)
          return
        }
