...
import Jolokia, { JolokiaRequest, JolokiaResponse, JolokiaSuccessResponse } from 'jolokia.js'
...
const MemoryView: React.FunctionComponent = () => {
  ...
  useEffect(() => {
    ...
    const register = async (request: JolokiaRequest, callback: (response: JolokiaResponse) => void) => {
      handle = await jolokiaService.register(request, callback)
      log.debug(selectedNode.name, '- Register request: handle =', handle)
    }
    register({ type: 'read', mbean, attribute: ['HeapMemoryUsage', 'NonHeapMemoryUsage'] }, (response: JolokiaResponse) => {
      if (Jolokia.isError(response)) {
        log.error(selectedNode.name, '- Scheduler - Error:', response.error)
        return
      }
